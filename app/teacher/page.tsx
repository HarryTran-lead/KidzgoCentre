  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        <StatCard icon={<BookOpen className="text-pink-600" size={18} />} label="Lớp đang dạy" value="5" hint="Ổn định" color="pink" />
        <StatCard icon={<CalendarClock className="text-emerald-600" size={18} />} label="Buổi/tuần này" value="12" hint="+2 so với tuần trước" color="green" />
        <StatCard icon={<Users className="text-amber-600" size={18} />} label="Giờ công tháng này" value="68h" hint="+5h" color="yellow" />
        <StatCard icon={<TrendingUp className="text-sky-600" size={18} />} label="Tổng học viên" value="82" hint="+3 mới" color="blue" />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <div className="rounded-2xl border bg-white p-4">
          ...
          <div className="divide-y">
            {[
              { name: "IELTS Foundation - A1", time: "08:00 - 10:00 • Phòng 301", tag: <Badge color="blue">Hôm nay</Badge>, students: 18 },
              { name: "TOEIC Intermediate", time: "14:00 - 16:00 • Phòng 205", tag: <Badge color="blue">Hôm nay</Badge>, students: 15 },
              { name: "Business English", time: "09:00 - 11:00 • Phòng 102", tag: <Badge color="gray">Thứ 6, 10/10</Badge>, students: 12 },
            ].map((r, i) => (
              <div key={i} className="py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-xs text-slate-500">{r.students} học viên</div>
                  {r.tag}
                </div>
              </div>
            ))}
          ...
          {[
            ...
            <div key={r.name}>
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="text-slate-500">Tiến độ: {r.progress}% • Chuyên cần: {r.attend}%</div>
              </div>
              <div className="h-2 bg-slate-100 rounded-full mt-2">
                <div className="h-2 bg-slate-900 rounded-full" style={{ width: `${r.progress}%` }} />
              </div>
            </div>
          ))}
        </div>
