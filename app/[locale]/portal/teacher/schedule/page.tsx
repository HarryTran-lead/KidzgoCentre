...
function checkTimeConflicts(lessons: Lesson[]): {
  hasConflict: boolean;
  conflicts: Array<{ lesson1: Lesson; lesson2: Lesson }>;
} {
...
function removeOverlappingLessons(lessons: Lesson[]): Lesson[] {
  ...
  for (const lesson of sortedLessons) {
    ...
    const overlapsWithValid = validLessons.some((validLesson) =>
      isTimeOverlap(validLesson.time, lesson.time),
    );
...
function groupOverlappingLessons(
  lessons: Lesson[],
): Map<
  string,
  { groupIndex: number; positionInGroup: number; totalInGroup: number }
> {
  ...
  for (let i = 0; i < lessons.length; i++) {
    ...
    for (let j = i + 1; j < lessons.length; j++) {
      ...
      const overlapsWithGroup = overlappingGroup.some((l) =>
        isTimeOverlap(l.time, lessons[j].time),
      );
...
function TimelineLesson({
  ...
  const [startHour, startMin] = lesson.time
    .split(" - ")[0]
    .split(":")
    .map(Number);
  ...
  const isHexColor =
    lesson.color.startsWith("#") || lesson.color.startsWith("rgb");
  ...
  const lightBgStyle = isHexColor
    ? { backgroundColor: `${lesson.color}33` }
    : undefined;
  ...
  return (
    <div
      ...
      title={
        hasConflict
          ? "⚠️ Cảnh báo: Buổi học này trùng giờ với buổi học khác trong ngày"
          : ""
      }
    >
      <div
        className={`h-1.5 w-full ${isHexColor ? "" : lesson.color}`}
        style={accentStyle}
      />
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={() => {
                  ...
                  router.push(
                    `/${locale}/portal/teacher/attendance?sessionId=${lesson.id}&date=${lessonDate}`,
                  );
                }}
...
function DayTimeline({
  ...
  const validLessons = useMemo(
    () => removeOverlappingLessons(data.lessons),
    [data.lessons],
  );
  const conflictCheck = useMemo(
    () => checkTimeConflicts(data.lessons),
    [data.lessons],
  );
  ...
  const lessonLayouts = useMemo(
    () => groupOverlappingLessons(validLessons),
    [validLessons],
  );
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-gray-600">
                {data.month} - {formatVNDate(data.date)}
              </div>
              {isToday && (
              ...
              {conflictCheck.hasConflict && (
                <div className="flex items-center gap-1 mt-2">
                  <div className="text-xs font-semibold text-red-600 bg-red-100 px-2 py-0.5 rounded-full flex items-center gap-1 border border-red-300">
                    <span>⚠️</span>
                    <span>
                      Đã loại bỏ {removedLessons.length} buổi học trùng giờ
                    </span>
                  </div>
                </div>
              )}
...
function WeekCalendarView({
  ...
  const { start: weekStart, end: weekEnd } = getWeekRangeByOffset(
    now,
    weekOffset,
  );
  ...
  const getLessonsForShift = (
    day: DaySchedule,
    shiftLabel: string,
  ): Lesson[] => {
    return day.lessons.filter(
      (lesson) => getShiftForLesson(lesson) === shiftLabel,
    );
  };
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
      <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-red-50 to-red-100">
        <div className="flex items-center gap-4">
          <div
            className={`relative p-3 rounded-xl bg-gradient-to-r from-red-600 to-red-700 text-white shadow-lg`}
          >
            <CalendarDays size={24} />
            <div className="absolute -top-1 -right-1 w-6 h-6 rounded-full bg-white flex items-center justify-center">
              <span className="text-xs font-bold text-red-600">
          ...
          <div>
            ...
            <div className="text-sm text-gray-900">
              {formatVNShortWithYear(weekStart)} –{" "}
              {formatVNShortWithYear(weekEnd)}
            </div>
          </div>
        </div>
          ...
          <GoToDateButton
            onSelect={(date) => {
              ...
              const offset = Math.round(
                (newWeekStart.getTime() - startOfWeek(now).getTime()) /
                  (7 * 24 * 60 * 60 * 1000),
              );
              onSetWeek(offset);
            }}
        ...
        >
          <div className="px-4 py-4 text-sm font-semibold text-gray-800 bg-gradient-to-r from-red-50 to-red-100 flex items-center justify-center">
            <div className="flex flex-col items-center">
              ...
              {shift.label === "Sáng" && (
                <span className="text-xs text-gray-600 mt-1">7:00-12:00</span>
              )}
              {shift.label === "Chiều" && (
                <span className="text-xs text-gray-600 mt-1">12:00-18:00</span>
              )}
              {shift.label === "Tối" && (
                <span className="text-xs text-gray-600 mt-1">18:00-22:00</span>
              )}
              ...
              >
                <div className="space-y-2">
                  {lessons.length === 0 ? (
                    ...
                    lessons.map((lesson) => {
                      const isHex =
                        lesson.color.startsWith("#") ||
                        lesson.color.startsWith("rgb");
                      const lightColor = isHex
                        ? ""
                        : lesson.color
                            .replace("bg-gradient-to-r", "bg-gradient-to-br")
                            .replace(
                            ...
                            .replace(
                              "from-red-500 to-gray-600",
                              "from-red-50 to-gray-100",
                            );
                      const lightBgStyle = isHex
                        ? { backgroundColor: `${lesson.color}33` }
                        : undefined;
                      const accentStyle = isHex
                        ? { backgroundColor: lesson.color }
                        : undefined;
                      ...
                      return (
                        <div key={lesson.id} className="relative group">
                          <div
                            onClick={() =>
                              router.push(
                                `/${locale}/portal/teacher/attendance?sessionId=${lesson.id}&date=${day.date}`,
                              )
                            }
                          ...
                          >
                            <div
                              className={`h-1.5 w-full ${isHex ? "" : lesson.color}`}
                              style={accentStyle}
                            />
                            <div className="p-2.5">
                              <div className="flex items-start gap-2">
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-1.5 mb-1">
                                    <span
                                      className={`h-2.5 w-2.5 rounded-full flex-shrink-0 ${isHex ? "" : lesson.color}`}
                                      style={accentStyle}
                                    />
...
export default function Page() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      total: weekData.reduce((sum, day) => sum + day.lessons.length, 0),
      byClass,
    };
  }, [weekData]);

  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-50 p-2 space-y-6">
      {/* Header */}
      <div
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1 flex items-center gap-2">
                ...
                Quản lý và theo dõi lịch dạy theo tuần với 3 ca Sáng – Chiều –
                Tối
              </p>
            </div>
          </div>
      ...
      {/* Filter Section */}
      <div
        className={`rounded-2xl border border-gray-200 bg-white p-4 flex flex-wrap gap-2 transition-all duration-700 delay-100 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
      >
        <div className="flex items-center gap-2">
        ...
        >
          <span>Tất cả</span>
          <span
            className={`text-xs px-1.5 py-0.5 rounded-full ${classFilter === "ALL" ? "bg-white/20" : "bg-gray-100 text-gray-700"}`}
          >
            {stats.total}
          </span>
        </button>
        ...
        {classOptions.slice(0, 4).map((c) => (
          ...
          >
            {c.name}
            <span
              className={`ml-2 text-xs px-1.5 py-0.5 rounded-full ${classFilter === c.id ? "bg-white/20" : "bg-gray-100 text-gray-700"}`}
            >
                ...
                {weekDaysForSelector.map((day) => (
                  ...
                  >
                    <div className="text-center">
                      <div
                        className={`text-xs ${selectedDate === day.date ? "text-white/90" : "text-gray-500"}`}
                      >
                        {day.dow === "Chủ nhật"
                          ? "CN"
                          : day.dow.replace("Th ", "Thứ ")}
                      ...
                      <div
                        className={`text-lg font-bold mt-0.5 ${selectedDate === day.date ? "text-white" : "text-gray-900"}`}
                      >
                        {day.day}
                      </div>
                      <div
                        className={`text-[10px] mt-0.5 ${selectedDate === day.date ? "text-white/80" : "text-gray-600"}`}
                      >
                        {day.month}
                      </div>
                      {day.lessons.length > 0 && (
                        <div
                          className={`text-[10px] mt-1 px-1.5 py-0.5 rounded-full ${selectedDate === day.date ? "bg-white/20" : "bg-red-100 text-red-600"}`}
                        >
      ...
      {/* Legend */}
      <div
        className={`mt-8 pt-6 border-t border-gray-200 transition-all duration-700 delay-300 ${isLoaded ? "opacity-100" : "opacity-0"}`}
      >
        <div className="flex items-center justify-between">
