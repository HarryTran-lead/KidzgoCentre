...
import { changeSessionSectionType } from "@/app/api/admin/sessions";
import { useToast } from "@/hooks/use-toast";
import { SECTION_TYPE_OPTIONS, SECTION_TYPE_LABELS } from "@/types/admin/sessions";
...
export default function LessonAttendancePage() {
  ...
  const isSessionToday = useMemo(() => {
    ...
    const parts = String(lesson.date).split("/").map((p) => p.trim());
    if (parts.length !== 3) return false;
    const [dd, mm, yyyy] = parts;
    const sessionDate = new Date(Number(yyyy), Number(mm) - 1, Number(dd));
  ...
  const handleSectionTypeSave = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: "Không thể cập nhật", description: err?.message || "Không thể đổi loại buổi học.", variant: "destructive" });
    } finally {
      setIsUpdatingSectionType(false);
    }
  }, [lesson?.sectionType, lessonId, pendingSectionType, toast]);
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="mt-3 space-y-2">
                ...
                {!isSessionToday && (
                  <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5">Chỉ có thể đổi loại buổi trong ngày học.</p>
                )}
                  ...
                  {SECTION_TYPE_OPTIONS.map((opt) => {
                    ...
                    return (
                      <button
                        key={opt.value}
                        type="button"
                        disabled={isUpdatingSectionType || !isSessionToday}
                ...
                {(lesson.sectionType ?? "Normal") !== pendingSectionType ? (
                  ...
                  <span className="text-xs text-gray-500">Loại hiện tại: {SECTION_TYPE_LABELS[lesson.sectionType as SectionType] ?? lesson.sectionType ?? "Normal"}</span>
                )}
              </div>
            </div>
          </div>
