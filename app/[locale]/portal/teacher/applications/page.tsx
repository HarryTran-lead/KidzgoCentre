...
export default function TeacherFeedbackPage() {
  ...
  const filtered = useMemo(() => {
    return tickets.filter((t) => {
      const mS =
        statusFilter === "Tất cả" || mapStatus(t.status) === statusFilter;
    ...
    <div className="min-h-screen bg-gradient-to-b from-red-50/30 to-white p-2 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1 flex items-center gap-2">
              ...
              Theo dõi và phản hồi các ticket được giao cho giáo viên
            </p>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-red-200 bg-gradient-to-br from-white to-red-50">
        ...
        <div className="flex flex-wrap items-center gap-2 p-4">
          {statusOpts.map((s) => {
            const count = s === "Tất cả" 
              ? stats.total
              : s === "Mới"
              ? stats.fresh
              : s === "Đang xử lý"
              ? stats.inProg
              : s === "Đã phản hồi" || s === "Đã đóng"
              ? stats.done
              : 0;
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl border transition-all duration-200 font-medium text-sm cursor-pointer ${
                  statusFilter === s
              ...
              >
                {s}
                <span className={`inline-flex items-center justify-center min-w-5 px-2 rounded-full text-xs font-bold ${
                  statusFilter === s
          ...
          <div className="flex flex-col md:flex-row items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                <SelectTrigger className="w-40 rounded-xl border border-red-300 bg-white px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-200 focus:border-red-400">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {catOpts.map((c) => (
                    <SelectItem key={c} value={c}>
                      {c === "Tất cả" ? c : CATEGORY_LABELS[c] || c}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
                ...
                {sorted.length > 0 ? (
                  sorted.map((t) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-2">
                          <div className="h-7 w-7 rounded-lg bg-gradient-to-r from-red-600 to-red-700 flex items-center justify-center text-white text-xs font-bold">
                            <User size={14} />
                          </div>
                      ...
                      <td className="py-4 px-6">
                        <button
                          ...
                          className="p-1.5 rounded-lg border border-red-300 bg-gradient-to-r from-white to-red-50 text-gray-500 hover:text-red-600 hover:bg-red-50 transition-colors cursor-pointer disabled:opacity-50"
                          title="Xem chi tiết"
                        >
                          <Eye size={14} />
                        </button>
