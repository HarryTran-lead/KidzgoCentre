...
export default function AdminFaqsPage() {
  ...
  const [catInactiveFilter, setCatInactiveFilter] = useState<"active" | "all">("all");
  const [catDeletedFilter, setCatDeletedFilter] = useState<"active" | "all">("active");
  ...
  const [itemFilterDeleted, setItemFilterDeleted] = useState<"active" | "all">("active");
  ...
  // ── Fetch categories ──
  const includeInactiveCats = catInactiveFilter === "all";
  const includeDeletedCats = catDeletedFilter === "all";
  ...
  // ── Fetch items ──
  const itemIncludeDeleted = itemFilterDeleted === "all";
  
  const fetchItems = useCallback(async () => {
    try {
      setItemsLoading(true);
      const params: Record<string, string | number | boolean> = { pageNumber: itemPage, pageSize: itemPageSize, includeDeleted: itemIncludeDeleted };
  ...
  useEffect(() => { setItemPage(1); }, [itemSearchTerm, itemFilterCategory, itemFilterPublished, itemFilterDeleted]);
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="relative flex-1 min-w-0">
                  ...
                  <input
                    ...
                    className="h-10 w-full rounded-xl border border-red-200 bg-white pl-9 pr-4 text-sm focus:border-red-300 focus:outline-none focus:ring-2 focus:ring-red-200 transition-colors"
                  />
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <Select value={catInactiveFilter} onValueChange={(val) => setCatInactiveFilter(val as "active" | "all")}>
                    <SelectTrigger className="h-10 w-48 rounded-xl border border-red-200 bg-white px-3 text-sm text-gray-700 hover:border-red-300 focus:ring-2 focus:ring-red-200">
                      <SelectValue placeholder="Trạng thái hoạt động" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="active">Chỉ hoạt động</SelectItem>
                      <SelectItem value="all">Tất cả</SelectItem>
                    </SelectContent>
                  </Select>
                  <Select value={catDeletedFilter} onValueChange={(val) => setCatDeletedFilter(val as "active" | "all")}>
                    <SelectTrigger className="h-10 w-40 rounded-xl border border-red-200 bg-white px-3 text-sm text-gray-700 hover:border-red-300 focus:ring-2 focus:ring-red-200">
                      <SelectValue placeholder="Trạng thái xóa" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="active">Chưa xóa</SelectItem>
                      <SelectItem value="all">Tất cả</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                  ...
                  <table className="w-full">
                    <thead className="bg-linear-to-r from-red-500/5 to-red-700/5 border-b border-red-200">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Thứ tự</th>
                        <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">FAQ (Xuất bản / Tổng)</th>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <Select value={itemFilterPublished} onValueChange={setItemFilterPublished}>
                      <SelectTrigger className="h-10 w-44 rounded-xl border border-red-200 bg-white px-3 text-sm text-gray-700 hover:border-red-300 focus:ring-2 focus:ring-red-200">
                    ...
                    <Select value={itemFilterDeleted} onValueChange={(val) => setItemFilterDeleted(val as "active" | "all")}>
                      <SelectTrigger className="h-10 w-40 rounded-xl border border-red-200 bg-white px-3 text-sm text-gray-700 hover:border-red-300 focus:ring-2 focus:ring-red-200">
                        <SelectValue placeholder="Trạng thái xóa" />
                      </SelectTrigger>
                      <SelectContent>
