...
export async function GET(req: Request) {
  ...
  const url = buildApiUrl(`${BACKEND_TICKET_TYPE_COMPATIBILITY_ENDPOINTS.BASE}${qs ? `?${qs}` : ""}`);
...
export async function POST(req: Request) {
  ...
  const upstream = await fetch(buildApiUrl(BACKEND_TICKET_TYPE_COMPATIBILITY_ENDPOINTS.BASE), {
    method: "POST",
    headers: { Authorization: authHeader, "Content-Type": "application/json" },
