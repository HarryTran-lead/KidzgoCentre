...
export async function PATCH(req: Request) {
  try {
    ...
    const upstream = await fetch(buildApiUrl(BACKEND_SESSION_ENDPOINTS.CHANGE_ROOM), {
      method: "PATCH",
      headers: {
    ...
    console.error("Change session room error:", error);
    return NextResponse.json(
      { isSuccess: false, data: null, message: "Đã xảy ra lỗi khi đổi phòng học" },
      { status: 500 }
    );
  }
}