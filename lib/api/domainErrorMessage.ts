...
const CODE_TO_VIETNAMESE_MESSAGE: Record<string, string> = {
  ...
  "Registration.ProgramNotAvailableInBranch": "Chương trình chưa được cấu hình cho chi nhánh đã chọn.",
  ...
  "Registration.SecondarySupplementaryRequiresSeparateRegistration": "Chương trình supplementary phải tạo đăng ký riêng.",
  ...
  "Registration.InvalidEntryType": "Hình thức vào lớp không hợp lệ.",
  ...
  "Registration.ClassNotMatchingBranch": "Lớp không thuộc chi nhánh của đăng ký.",
  "Registration.ActualStartDateInFuture": "Ngày bắt đầu thực tế không được lớn hơn hiện tại.",
  "Registration.ImportSessionCountMismatch": "Tổng buổi đã học và còn lại không khớp với gói học.",
  "Registration.FirstStudyDateNotAllowed": "Không thể nhập ngày học đầu tiên cho hình thức vào lớp hiện tại.",
  "Registration.FirstStudyDateBeforeClassStart": "Ngày học đầu tiên không được trước ngày bắt đầu lớp.",
  "Registration.FirstStudyDateAfterClassEnd": "Ngày học đầu tiên không được sau ngày kết thúc lớp.",
  "Registration.FirstStudyDateNoSession": "Ngày học đầu tiên không khớp buổi học thực tế của lớp.",
  "Registration.InvalidEnrollmentConfirmationPdfFormType": "Loại mẫu phiếu xác nhận không hợp lệ.",
  ...
  "WeeklyPattern.InvalidDuration": "Thời lượng buổi học phải lớn hơn 0.",
  "WeeklyPattern.InvalidDayOfWeek": "Ngày học trong weeklyPattern không hợp lệ.",
  "WeeklyPattern.InvalidStartTime": "Giờ bắt đầu trong weeklyPattern không hợp lệ.",
  ...
  "Enrollment.AlreadyEnrolled": "Học viên đã có ghi danh Active/Paused ở lớp này.",
...
function translateBackendMessageToVietnamese(message?: string): string | undefined {
  ...
  const messageMatchers: Array<{ test: RegExp; vi: string }> = [
    ...
    {
      test: /already\s+has\s+a\s+class\s+assigned.*use\s+transfer-?class\s+instead/i,
      vi: "Track này đã được xếp lớp. Vui lòng dùng chức năng chuyển lớp nếu cần thay đổi.",
    },
    {
    ...
    {
      test: /sessions?\s+must\s+be\s+at\s+least\s+\d+\s+minutes?\s+apart/i,
      vi: "Các buổi học của học viên đang quá sát nhau. Vui lòng chọn lớp/khung giờ để các buổi cách nhau ít nhất 15 phút.",
    },
    {
...
export function extractDomainErrorCode(error: unknown): string | undefined {
  ...
  const candidates = [
    normalizeCode(e?.response?.data?.title),
