...
export function LearnerGamificationWorkspace({
  ...
  return (
    <div className="space-y-6 bg-[radial-gradient(circle_at_top,_rgba(251,191,36,0.18),_transparent_38%),linear-gradient(180deg,_#fffdf7,_#f8fafc_55%)] p-4 md:p-6">
      <div className="rounded-[32px] border border-amber-200/60 bg-white/90 p-6 shadow-lg shadow-amber-100/40">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-amber-700"><Sparkles className="h-4 w-4" />Gamification cho {portalLabel.toLowerCase()}</div>
            <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-900">Hành trình tích sao và chinh phục nhiệm vụ</h1>
            <p className="mt-2 max-w-3xl text-sm text-slate-600">{activeStudent?.displayName ? `Đang theo dõi hồ sơ: ${activeStudent.displayName}.` : "Theo dõi nhiệm vụ, streak điểm danh, sao, XP và yêu cầu đổi quà tại một nơi."}</p>
          </div>
          <button type="button" onClick={handleCheckIn} disabled={busyAction === "checkin"} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:opacity-60">{busyAction === "checkin" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Flame className="h-4 w-4" />}Điểm danh hôm nay</button>
        </div>
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <MetricCard icon={<Star className="h-5 w-5" />} label="Số sao hiện có" value={formatNumber(starBalance)} hint="Dùng để đổi quà" accent="from-amber-500 via-orange-500 to-red-500" />
          <MetricCard icon={<TrendingUp className="h-5 w-5" />} label="XP hiện tại" value={formatNumber(levelInfo?.xp)} hint={`Còn ${formatNumber(levelInfo?.xpRequiredForNextLevel)} XP để lên cấp`} accent="from-sky-500 via-cyan-500 to-blue-600" />
          ...
          <MetricCard icon={<Flame className="h-5 w-5" />} label="Streak hiện tại" value={`${formatNumber(streakInfo?.currentStreak)} ngày`} hint={`Kỷ lục: ${formatNumber(streakInfo?.maxStreak)} ngày`} accent="from-rose-500 via-orange-500 to-amber-500" />
        </div>
      </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.25fr_0.95fr]">
          <Panel>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <MetricCard icon={<Target className="h-5 w-5" />} label="Mission đang mở" value={formatNumber(missions.length)} accent="from-slate-700 via-slate-900 to-black" />
            ...
            <div className="mt-5 space-y-3">
              {missions.slice(0, 3).map((mission) => (
                <div key={mission.id} className="flex items-start justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div><p className="font-semibold text-slate-900">{mission.title}</p><p className="mt-1 text-sm text-slate-500">{mapMissionTypeLabel(mission.missionType)} • {mapMissionScopeLabel(mission.scope)}</p></div>
                  <button type="button" onClick={() => void openMissionProgress(mission)} className="rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-white">Xem tiến độ</button>
                </div>
              ))}
            ...
            {(streakInfo?.recentStreaks ?? []).length ? (
              <div className="space-y-3">
                {streakInfo!.recentStreaks.slice(0, 6).map((item) => (
                  <div key={item.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-center justify-between gap-3"><div><p className="font-semibold text-slate-900">Điểm danh ngày {formatDate(item.attendanceDate)}</p><p className="mt-1 text-sm text-slate-500">Streak: {item.currentStreak} ngày</p></div><div className="text-right"><p className="font-semibold text-amber-600">+{item.rewardStars} sao</p><p className="text-sm font-medium text-sky-600">+{item.rewardExp} XP</p></div></div></div>
                ))}
              </div>
          ...
          {activeTab === "missions" ? (
            <>
              ...
              {missions.length ? missions.map((mission) => (
                <div key={mission.id} className="mb-4 rounded-3xl border border-slate-200 bg-slate-50 p-5 last:mb-0">
                  <div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-bold text-slate-900">{mission.title}</h3><p className="mt-1 text-sm text-slate-500">{mapMissionTypeLabel(mission.missionType)} • {mapMissionScopeLabel(mission.scope)}</p>{mission.description ? <p className="mt-3 text-sm leading-6 text-slate-600">{mission.description}</p> : null}</div><button type="button" onClick={() => void openMissionProgress(mission)} disabled={busyAction === `mission-${mission.id}`} className="inline-flex items-center gap-2 rounded-2xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-white disabled:opacity-60">{busyAction === `mission-${mission.id}` ? <Loader2 className="h-4 w-4 animate-spin" /> : <BookOpen className="h-4 w-4" />}Xem tiến độ</button></div>
                </div>
          ...
          {activeTab === "streak" ? (
            <>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                ...
                <MetricCard icon={<Medal className="h-5 w-5" />} label="Streak cao nhất" value={`${formatNumber(streakInfo?.maxStreak)} ngày`} accent="from-amber-500 via-yellow-500 to-lime-500" />
        ...
        <div className="grid gap-6 xl:grid-cols-[1.25fr_0.95fr]">
                ...
                {rewards.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                      <div className="bg-[linear-gradient(120deg,_rgba(14,165,233,0.12),_rgba(251,191,36,0.22),_rgba(244,63,94,0.12))] px-5 py-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-bold text-slate-900">{item.title}</h3><p className="mt-1 text-sm text-slate-600">{item.description || "Vật phẩm thưởng đang có trong cửa hàng."}</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-700 shadow-sm">Còn {formatNumber(item.quantity)}</span></div></div>
                      <div className="space-y-4 p-5"><div className="rounded-2xl bg-slate-50 px-4 py-3"><p className="text-xs uppercase tracking-wide text-slate-500">Giá đổi</p><p className="mt-1 text-xl font-black text-amber-600">{formatNumber(item.costStars)} sao</p></div><button type="button" onClick={() => { setRedeemItem(item); setRedeemQuantity(1); }} disabled={disabled} className={disabled ? "inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-400" : "inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800"}><Gift className="h-4 w-4" />{item.quantity <= 0 ? "Đã hết quà" : starBalance < item.costStars ? "Không đủ sao" : "Đổi quà ngay"}</button></div>
                    </div>
                  );
                })}
              </div>
          ...
          <Panel>
            ...
            {redemptions.length ? redemptions.map((item) => (
              <div key={item.id} className="mb-3 rounded-3xl border border-slate-200 bg-slate-50 p-4 last:mb-0">
                <div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold text-slate-900">{item.itemName}</h3><p className="mt-1 text-sm text-slate-500">SL {item.quantity} • {formatDateTime(item.createdAt)}</p><p className="mt-1 text-sm text-slate-500">Sao đã trừ: {formatNumber(item.starsDeducted)}</p></div><StatusPill label={mapRedemptionStatusLabel(item.status)} className={getRedemptionStatusClasses(item.status)} /></div>
                {item.status === "Delivered" ? <button type="button" onClick={() => void confirmReceived(item.id)} disabled={busyAction === `confirm-${item.id}`} className="mt-4 inline-flex items-center gap-2 rounded-2xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-60">{busyAction === `confirm-${item.id}` ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}Xác nhận đã nhận quà</button> : null}
              </div>
      ...
      <DialogShell open={Boolean(selectedMission)} title={selectedMission?.title ?? "Tiến độ mission"} description="Tiến độ theo hồ sơ học sinh đang được chọn." onClose={() => { setSelectedMission(null); setMissionProgress([]); }}>
        {missionProgress.length ? missionProgress.map((item) => (
          <div key={item.id} className="mb-3 rounded-3xl border border-slate-200 bg-slate-50 p-5 last:mb-0"><div className="flex items-start justify-between gap-3"><div><h4 className="font-semibold text-slate-900">{item.studentName || activeStudent?.displayName || "Học sinh đang chọn"}</h4><p className="mt-1 text-sm text-slate-500">Tiến độ: {formatNumber(item.progressValue)} • {formatNumber(item.progressPercentage)}%</p><p className="mt-1 text-sm text-slate-500">Hoàn tất lúc: {formatDateTime(item.completedAt)}</p></div><StatusPill label={mapProgressStatusLabel(item.status)} className={getMissionProgressClasses(item.status)} /></div></div>
      ...
      <DialogShell open={Boolean(redeemItem)} title={`Đổi thưởng: ${redeemItem?.title ?? ""}`} description="Hệ thống sẽ trừ sao và giữ tồn kho ngay khi tạo yêu cầu." onClose={() => { setRedeemItem(null); setRedeemQuantity(1); }} widthClass="max-w-xl">
        {redeemItem ? (
          <div className="space-y-5">
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h4 className="text-lg font-bold text-slate-900">{redeemItem.title}</h4><p className="mt-2 text-sm leading-6 text-slate-600">{redeemItem.description || "Không có mô tả chi tiết cho vật phẩm này."}</p></div>
            <div><label className="mb-2 block text-sm font-semibold text-slate-700">Số lượng muốn đổi</label><input type="number" min={1} max={Math.max(1, redeemItem.quantity)} value={redeemQuantity} onChange={(event) => setRedeemQuantity(Math.max(1, Number(event.target.value || 1)))} className="w-full rounded-2xl border border-slate-300 px-4 py-3 text-slate-900 outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200" /></div>
            <div className="rounded-3xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">Tổng sao sẽ bị trừ ngay: <span className="font-bold">{formatNumber(redeemItem.costStars * redeemQuantity)} sao</span></div>
            <div className="flex items-center justify-end gap-3"><button type="button" onClick={() => { setRedeemItem(null); setRedeemQuantity(1); }} className="rounded-2xl border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Hủy</button><button type="button" onClick={() => void redeem()} disabled={busyAction === `redeem-${redeemItem.id}`} className="inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:opacity-60">{busyAction === `redeem-${redeemItem.id}` ? <Loader2 className="h-4 w-4 animate-spin" /> : <Gift className="h-4 w-4" />}Xác nhận đổi thưởng</button></div>
          </div>
        ) : null}
      </DialogShell>
    </div>
